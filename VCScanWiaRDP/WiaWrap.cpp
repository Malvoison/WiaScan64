...
HRESULT ReadPropertyBSTR(IWiaPropertyStorage* pWiaPropertyStorage, PROPID propid , BSTR* pbstr)
{
    ...
    *pbstr = NULL;
    ...
    if(S_OK == hr)
    {
        ...
        {
            ReportError(TEXT("Trying to read a property which doesn't return a BSTR"), hr);
        }
    }
    else
    {
...
HRESULT WiaSetPreferences(HWND hWndParent)
{
	HRESULT hr;
	CComPtr<IStream> pPropStream;

	CComPtr<IWiaDevMgr> pWiaDevMgr = NULL;
	hr = pWiaDevMgr.CoCreateInstance(CLSID_WiaDevMgr);
	if (FAILED(hr))
	{
		return hr;
	...
	hr = pWiaDevMgr->SelectDeviceDlg(hWndParent, StiDeviceTypeScanner, 0, NULL,
		&pItemRoot);
	...
	hr = pItemRoot->DeviceDlg(hWndParent, 0, WIA_INTENT_IMAGE_TYPE_GRAYSCALE,
		&ppIWiaItem.Count(), &ppIWiaItem);
	...
	//  perform useful storage of preferences here
	CComQIPtr<IWiaPropertyStorage> pPropStorage(pItemRoot);
	BSTR bstr;
	ReadPropertyBSTR(pPropStorage, WIA_DIP_DEV_ID, &bstr);
	CComBSTR bstrDevId;
	bstrDevId.Attach(bstr);
	//  Perform useful tasks with the BSTR
	HKEY hkScanKey = NULL;
	...
	{
		MessageBox(g_hWndMstsc, L"Error configuring EHS Remote Scanning.  Please contact your system administrator.",
			L"Configuration Error", MB_ICONSTOP | MB_OK);
		return E_FAIL;
	}
	
	HKEY hkDevId;
	lResult = RegCreateKey(hkScanKey, bstrDevId, &hkDevId);
	if (lResult != ERROR_SUCCESS)
	{
		MessageBox(g_hWndMstsc, L"Error configuring EHS Remote Scanning.  Please contact your system administrator.",
			L"Configuration Error", MB_ICONSTOP | MB_OK);
	...
	CComQIPtr<IWiaPropertyStorage> pPropStorageItem(ppIWiaItem[0]);
	GUID guidProp;
	hr = pPropStorageItem->GetPropertyStream(&guidProp, &pPropStream);
	BYTE lpBuffer[2048];
	RtlZeroMemory(lpBuffer, 2048);
	ULONG ulRead;
	LARGE_INTEGER li;
	_int64 pos = 0;
	li.QuadPart = pos;
	hr = pPropStream->Seek(li, STREAM_SEEK_SET, NULL);
	hr = pPropStream->Read(lpBuffer, 2048, &ulRead);

	lResult = RegSetValueEx(hkDevId, L"Properties", 0, REG_BINARY, (const LPBYTE)lpBuffer, ulRead);
	if (lResult != ERROR_SUCCESS)
	{
		MessageBox(g_hWndMstsc, L"Error configuring EHS Remote Scanning.  Please contact your system administrator.",
			L"Configuration Error", MB_ICONSTOP | MB_OK);
...
)
{
    ...
    *pppStream = NULL;
	...
	//  Check the registry for existing properties in the registry
	/*CComQIPtr<IWiaPropertyStorage> pRootStorage(pItemRoot);*/
	CComPtrArray<IWiaItem> ppIWiaItem;
	//BSTR bstr;
	//ReadPropertyBSTR(pRootStorage, WIA_DIP_DEV_ID, &bstr);
	//CComBSTR bstrDevId;
	//bstrDevId.Attach(bstr);	
	//HKEY hKeyBase;
	//HKEY hkDevId;
	//const wchar_t* szBaseScanKey = 
	//	L"SOFTWARE\\Microsoft\\Terminal Server Client\\Default\\AddIns\\VCScanWiaRDP";
	//LONG lResult = RegOpenKey(HKEY_CURRENT_USER, szBaseScanKey, &hKeyBase);
	//if (lResult != ERROR_SUCCESS)
	//{
	//	MessageBox(g_hWndMstsc, L"Error configuring EHS Remote Scanning.  Please contact your system administrator.",
	//		L"Configuration Error", MB_ICONSTOP | MB_OK);
	...
	//lResult = RegOpenKey(hKeyBase, bstrDevId, &hkDevId);
	//if (lResult != ERROR_SUCCESS)
	//{
	...
	//else
	//{
	//	//  Read the Properties value from the registry
	//	LPBYTE lpProperties = NULL;
	//	DWORD dwSize;
	//	lResult = RegQueryValueEx(hkDevId, L"Properties", NULL, NULL, NULL, &dwSize);
	//	if (lResult == ERROR_SUCCESS)
	//	{
	//		HGLOBAL hGlobProps = GlobalAlloc(GHND, dwSize);
	//		lpProperties = (LPBYTE)GlobalLock(hGlobProps);
	//		lResult = RegQueryValueEx(hkDevId, L"Properties", NULL, NULL, lpProperties, &dwSize);
	//		GlobalUnlock(hGlobProps);
	//		//  Enum the children of the root
	//		CComPtr<IEnumWiaItem> pEnumWiaChildItem;
	//		hr = pItemRoot->EnumChildItems(&pEnumWiaChildItem);
	//		if (SUCCEEDED(hr))
	//		{
	//			IWiaItem* wiaChild;
	//			ULONG ulFetched, ulCount;
	//			pEnumWiaChildItem->GetCount(&ulCount);
	//			pEnumWiaChildItem->Reset();
	//			ppIWiaItem.SetSize(ulCount);
	//			hr = pEnumWiaChildItem->Next(ulCount, ppIWiaItem, (ULONG*)&ppIWiaItem.Count());

	//			CComQIPtr<IWiaPropertyStorage> pPropStorage(ppIWiaItem[0]);
	//			CComPtr<IStream> pPropStream;
	//			hr = CreateStreamOnHGlobal(hGlobProps, TRUE, &pPropStream);

	//			GUID guidProp;
	//			RtlZeroMemory(&guidProp, sizeof(GUID));
	//			hr = pPropStorage->SetPropertyStream(&guidProp, pPropStream);
	//		}
	//	}
	//	else
	//	{
