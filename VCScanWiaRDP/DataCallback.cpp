...
HRESULT CDataCallback::StoreBuffer()
{
    ...
    *m_pppStream = ppStream;
	...
	LPIMGLIST lpImg = (LPIMGLIST)HeapAlloc(hHeap, HEAP_ZERO_MEMORY, sizeof(IMGLIST));
	lpImg->img = (LPBYTE)HeapAlloc(hHeap, HEAP_ZERO_MEMORY, statStg.cbSize.LowPart);
	lpImg->nImageSize = (int)statStg.cbSize.LowPart;
	lpImg->nImageSequence = *m_plCount;
	RtlCopyMemory(lpImg->img, GlobalLock(hGlobal), statStg.cbSize.LowPart);

	CreateThread(NULL, 0, ImageConverterThreadProc, lpImg, 0, NULL);
